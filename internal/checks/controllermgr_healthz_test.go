...
func TestControllerMgrHealthzOKWhenLeaseFresh(t *testing.T) {
	...
	withNow(t, now)
	env := envWithObjects(lease("kube-system", controllerMgrLeaseName, "ctrl01", now.Add(-3*time.Second), 15))
	got := runCheck(t, &controllerMgrHealthz{}, env)
...
func TestControllerMgrHealthzCriticalWhenLeaseStale(t *testing.T) {
	now := time.Date(2026, 5, 9, 12, 0, 0, 0, time.UTC)
	withNow(t, now)
	env := envWithObjects(lease("kube-system", controllerMgrLeaseName, "ctrl01", now.Add(-2*time.Minute), 15))
	got := runCheck(t, &controllerMgrHealthz{}, env)
...
func TestControllerMgrHealthzCriticalWhenNoHolder(t *testing.T) {
	now := time.Date(2026, 5, 9, 12, 0, 0, 0, time.UTC)
	withNow(t, now)
	env := envWithObjects(lease("kube-system", controllerMgrLeaseName, "", now, 15))
	got := runCheck(t, &controllerMgrHealthz{}, env)
	if statusCounts(got)[result.StatusCritical] != 1 {
		t.Fatalf("want CRIT, got %+v", got)
	}
...
// lease builds a coordination.k8s.io/v1 Lease for tests.
func lease(ns, name, holder string, renew time.Time, durSec int32) *coordv1.Lease {
	l := &coordv1.Lease{
